...
def ground_grass(name="GroundGrass", dark=False):
    ...
    if dark:
        stops = [(0.18, (0.06, 0.10, 0.05, 1.0)),
                 (0.48, (0.13, 0.20, 0.09, 1.0)),
                 (0.78, (0.21, 0.29, 0.13, 1.0)),
                 (1.00, (0.29, 0.36, 0.18, 1.0))]
    else:
        stops = [(0.18, (0.17, 0.22, 0.09, 1.0)),
                 (0.48, (0.29, 0.35, 0.14, 1.0)),
                 (0.78, (0.41, 0.46, 0.20, 1.0)),
                 (1.00, (0.52, 0.55, 0.28, 1.0))]
    ramp = _ramp(nt, stops, clump.outputs["Fac"])

    # individual tufts, ~24 across the tile
    tuft = _noise(nt, pos, scale=24.0, detail=9.0, roughness=0.8, distortion=3.0)
    tuft_ramp = _ramp(nt, [
        (0.30, (0.68, 0.70, 0.62, 1.0)),
        (0.52, (1.00, 1.00, 1.00, 1.0)),
        (0.74, (1.24, 1.22, 1.12, 1.0)),
    ], tuft.outputs["Fac"])
...
def ground_scrub(name="GroundScrub"):
    ...
    ramp = _ramp(nt, [
        ...
        (0.44, (0.50, 0.47, 0.26, 1.0)),
        (0.62, (0.30, 0.36, 0.15, 1.0)),
        (0.80, (0.44, 0.44, 0.23, 1.0)),
